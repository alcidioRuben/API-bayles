...
export function makeLibSignalRepository(auth: SignalAuthState): SignalRepository {
	...
	return {
		...
		async decryptMessage({ jid, type, ciphertext }) {
			...
			const session = new (libsignal as any).SessionCipher(storage, addr)
			let result: Buffer
			switch (type) {
			case 'pkmsg':
		...
		async encryptMessage({ jid, data }) {
			const addr = jidToSignalProtocolAddress(jid)
			const cipher = new (libsignal as any).SessionCipher(storage, addr)
		...
		async injectE2ESession({ jid, session }) {
			const cipher = new (libsignal as any).SessionBuilder(storage, jidToSignalProtocolAddress(jid))
